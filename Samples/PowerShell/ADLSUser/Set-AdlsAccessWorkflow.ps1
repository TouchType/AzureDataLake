...
workflow adlsAccessSet {
       ...
       foreach -parallel ($path in $adlspaths){
        ...
        InlineScript
        {
            ...
            # Login-AzureRmAccount -ServicePrincipal -ApplicationId  $sp_app_id -Credential $credential -TenantId $tenantid
...
adlsAccessSet  -adlspaths $paths -sp_username $sp_username -sp_app_id $sp_app_id -sp_app_pass $sp_app_pass -tenantid $tenantid
